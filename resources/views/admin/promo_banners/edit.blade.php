...
<div class="max-w-2xl glass-panel p-8 rounded-3xl shadow-xl">
    <form method="POST" action="{{ route('admin.promo-banners.update', $promoBanner->id) }}" class="space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="space-y-4">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <label class="flex items-center space-x-2.5 cursor-pointer bg-gray-950/40 border border-gray-800 hover:border-gray-700 px-4 py-2.5 rounded-xl">
                    <input type="checkbox" name="is_active" value="1" {{ old('is_active', $promoBanner->is_active) ? 'checked' : '' }} class="rounded border-gray-800 bg-gray-950 text-cyan-500 focus:ring-cyan-500 w-4 h-4">
                ...
                <div>
                    ...
                    <input id="order" type="number" name="order" value="{{ old('order', $promoBanner->order) }}" required
        ...
        <div class="space-y-4 pt-4 border-t border-gray-800">
            ...
            <div>
                ...
                <input id="title" type="text" name="title" value="{{ old('title', $promoBanner->title) }}" required
                       class="w-full bg-gray-950/50 border border-gray-800 focus:border-cyan-500 focus:ring-1 focus:ring-cyan-500 text-white rounded-xl px-4 py-2.5 outline-none transition-all text-sm"
            ...
            <div>
                ...
                <input id="subtitle" type="text" name="subtitle" value="{{ old('subtitle', $promoBanner->subtitle) }}" required
                       class="w-full bg-gray-950/50 border border-gray-800 focus:border-cyan-500 focus:ring-1 focus:ring-cyan-500 text-white rounded-xl px-4 py-2.5 outline-none transition-all text-sm"
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <input id="logo" type="url" name="logo" value="{{ old('logo', $promoBanner->logo) }}"
                           class="w-full bg-gray-950/50 border border-gray-800 focus:border-cyan-500 focus:ring-1 focus:ring-cyan-500 text-white rounded-xl px-4 py-2.5 outline-none transition-all text-sm"
                ...
                <div>
                    ...
                    <input id="countdown" type="datetime-local" name="countdown" 
                           value="{{ old('countdown', $promoBanner->countdown ? date('Y-m-d\TH:i', strtotime($promoBanner->countdown)) : '') }}"
                           class="w-full bg-gray-950/50 border border-gray-800 focus:border-cyan-500 focus:ring-1 focus:ring-cyan-500 text-white rounded-xl px-4 py-2.5 outline-none transition-all text-sm">
                </div>
            </div>
        </div>
        ...
        <div class="space-y-4 pt-4 border-t border-gray-800">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <input id="btn_text" type="text" name="btn_text" value="{{ old('btn_text', $promoBanner->btn_text) }}"
                           class="w-full bg-gray-950/50 border border-gray-800 focus:border-cyan-500 focus:ring-1 focus:ring-cyan-500 text-white rounded-xl px-4 py-2.5 outline-none transition-all text-sm"
                ...
                <div>
                    ...
                    <input id="btn_link" type="url" name="btn_link" value="{{ old('btn_link', $promoBanner->btn_link) }}"
                           class="w-full bg-gray-950/50 border border-gray-800 focus:border-cyan-500 focus:ring-1 focus:ring-cyan-500 text-white rounded-xl px-4 py-2.5 outline-none transition-all text-sm"
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                    ...
                    <select id="stream1_id" name="stream1_id" class="w-full bg-gray-950/60 border border-gray-800 focus:border-cyan-500 focus:ring-1 focus:ring-cyan-500 text-white rounded-xl px-3 py-2 outline-none text-xs transition-all">
                        ...
                        @foreach($streams as $stream)
                            <option value="{{ $stream->id }}" {{ old('stream1_id', $promoBanner->stream1_id) == $stream->id ? 'selected' : '' }}>
                                {{ $stream->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <select id="stream2_id" name="stream2_id" class="w-full bg-gray-950/60 border border-gray-800 focus:border-cyan-500 focus:ring-1 focus:ring-cyan-500 text-white rounded-xl px-3 py-2 outline-none text-xs transition-all">
                        <option value="">-- None Selected --</option>
                        @foreach($streams as $stream)
                            <option value="{{ $stream->id }}" {{ old('stream2_id', $promoBanner->stream2_id) == $stream->id ? 'selected' : '' }}>
                                {{ $stream->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <select id="stream3_id" name="stream3_id" class="w-full bg-gray-950/60 border border-gray-800 focus:border-cyan-500 focus:ring-1 focus:ring-cyan-500 text-white rounded-xl px-3 py-2 outline-none text-xs transition-all">
                        <option value="">-- None Selected --</option>
                        @foreach($streams as $stream)
                            <option value="{{ $stream->id }}" {{ old('stream3_id', $promoBanner->stream3_id) == $stream->id ? 'selected' : '' }}>
                                {{ $stream->name }}
                            </option>
                        @endforeach
                    </select>
