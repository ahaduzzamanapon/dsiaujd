...
@section('page-subtitle', 'Modify stream details, category folders, and server link order')

@section('content')
<form method="POST" action="{{ route('admin.streams.update', $stream->id) }}" class="space-y-8 max-w-4xl">
    @csrf
    @method('PUT')
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <div class="glass-panel p-6 rounded-3xl space-y-6 shadow-xl">
            ...
            <div>
                ...
                <input id="name" type="text" name="name" value="{{ old('name', $stream->name) }}" required
            ...
            <div>
                ...
                <input id="logo" type="url" name="logo" value="{{ old('logo', $stream->logo) }}"
                       class="w-full bg-gray-950/50 border border-gray-800 focus:border-cyan-500 focus:ring-1 focus:ring-cyan-500 text-white placeholder-gray-600 rounded-xl px-4 py-2.5 outline-none transition-all text-sm">
            </div>
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <select id="sport_type" name="sport_type" required
                        ...
                        <option value="other" {{ old('sport_type', $stream->sport_type) == 'other' ? 'selected' : '' }}>Other / General</option>
                        <option value="cricket" {{ old('sport_type', $stream->sport_type) == 'cricket' ? 'selected' : '' }}>Cricket</option>
                        <option value="football" {{ old('sport_type', $stream->sport_type) == 'football' ? 'selected' : '' }}>Football</option>
                    </select>
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-300 mb-2">Status</label>
                    <label class="relative inline-flex items-center mt-2 cursor-pointer">
                        <input type="checkbox" name="is_active" value="1" {{ old('is_active', $stream->is_active) ? 'checked' : '' }} class="sr-only peer">
            ...
            <div class="pt-4 border-t border-gray-800 space-y-3">
                ...
                <div class="flex flex-wrap gap-4">
                    <label class="flex items-center space-x-2.5 cursor-pointer bg-gray-950/40 border border-gray-800 hover:border-gray-700 px-4 py-2 rounded-xl">
                        <input type="checkbox" name="show_in_events" id="show_in_events" value="1" {{ old('show_in_events', $stream->show_in_events) ? 'checked' : '' }} class="rounded border-gray-800 bg-gray-950 text-cyan-500 focus:ring-cyan-500">
                        <span class="text-sm text-gray-300">Live Events</span>
                    </label>
                    <label class="flex items-center space-x-2.5 cursor-pointer bg-gray-950/40 border border-gray-800 hover:border-gray-700 px-4 py-2 rounded-xl">
                        <input type="checkbox" name="show_in_sports" id="show_in_sports" value="1" {{ old('show_in_sports', $stream->show_in_sports) ? 'checked' : '' }} class="rounded border-gray-800 bg-gray-950 text-cyan-500 focus:ring-cyan-500">
                        <span class="text-sm text-gray-300">Sports Tab</span>
                    </label>
                    <label class="flex items-center space-x-2.5 cursor-pointer bg-gray-950/40 border border-gray-800 hover:border-gray-700 px-4 py-2 rounded-xl">
                        <input type="checkbox" name="show_in_tv" id="show_in_tv" value="1" {{ old('show_in_tv', $stream->show_in_tv) ? 'checked' : '' }} class="rounded border-gray-800 bg-gray-950 text-cyan-500 focus:ring-cyan-500" onchange="toggleCategoriesSection()">
            ...
            @php $syncedCats = $stream->categories->pluck('id')->toArray(); @endphp
            <div id="categories-section" class="{{ $stream->show_in_tv ? '' : 'hidden' }} pt-4 border-t border-gray-800 space-y-3">
                ...
                <div class="grid grid-cols-2 gap-2 max-h-40 overflow-y-auto p-2 bg-gray-950/30 rounded-xl border border-gray-800/80">
                    @foreach($categories as $category)
                        <label class="flex items-center space-x-2.5 cursor-pointer py-1">
                            <input type="checkbox" name="categories[]" value="{{ $category->id }}" {{ in_array($category->id, $syncedCats) ? 'checked' : '' }} class="rounded border-gray-800 bg-gray-950 text-cyan-500 focus:ring-cyan-500">
        ...
        <div class="glass-panel p-6 rounded-3xl space-y-6 shadow-xl h-fit">
            ...
            <div class="space-y-4">
                ...
                <label class="flex items-center space-x-2.5 cursor-pointer bg-gray-950/40 border border-gray-800 px-4 py-2.5 rounded-xl">
                    <input type="checkbox" name="is_permanent" id="is_permanent" value="1" {{ old('is_permanent', $stream->is_permanent) ? 'checked' : '' }} onchange="toggleExpirySection()" class="rounded border-gray-800 bg-gray-950 text-cyan-500 focus:ring-cyan-500">
                    <span class="text-sm text-gray-300">Permanent (Never Expires)</span>
                </label>

                <!-- Expiring Stream Date Times -->
                <div id="expiry-section" class="{{ $stream->is_permanent ? 'hidden' : '' }} space-y-4 pt-2">
                    <div>
                        ...
                        <input id="start_time" type="datetime-local" name="start_time" 
                               value="{{ $stream->start_time ? $stream->start_time->format('Y-m-d\TH:i') : '' }}"
                    ...
                    <div>
                        ...
                        <input id="expire_time" type="datetime-local" name="expire_time" 
                               value="{{ $stream->expire_time ? $stream->expire_time->format('Y-m-d\TH:i') : '' }}"
                               class="w-full bg-gray-950/50 border border-gray-800 focus:border-cyan-500 focus:ring-1 focus:ring-cyan-500 text-white rounded-xl px-4 py-2 outline-none transition-all text-sm">
                    </div>
            ...
            <div class="space-y-4 pt-4 border-t border-gray-800">
                ...
                <div class="grid grid-cols-2 gap-4">
                    ...
                    <div class="space-y-3 p-3 bg-gray-950/20 border border-gray-800/80 rounded-2xl">
                        ...
                        <input type="text" name="team1_name" value="{{ old('team1_name', $stream->team1_name) }}" placeholder="Name (e.g. BAN)" class="w-full bg-gray-950/40 border border-gray-800 focus:border-cyan-500 text-white rounded-lg px-2.5 py-1.5 outline-none text-xs">
                        <input type="url" name="team1_logo" value="{{ old('team1_logo', $stream->team1_logo) }}" placeholder="Flag/Logo URL" class="w-full bg-gray-950/40 border border-gray-800 focus:border-cyan-500 text-white rounded-lg px-2.5 py-1.5 outline-none text-xs">
                    </div>

                    <!-- Team 2 -->
                    <div class="space-y-3 p-3 bg-gray-950/20 border border-gray-800/80 rounded-2xl">
                        <span class="text-xs font-bold text-gray-400 block border-b border-gray-800 pb-1">Team 2 (Right)</span>
                        <input type="text" name="team2_name" value="{{ old('team2_name', $stream->team2_name) }}" placeholder="Name (e.g. AUS)" class="w-full bg-gray-950/40 border border-gray-800 focus:border-cyan-500 text-white rounded-lg px-2.5 py-1.5 outline-none text-xs">
                        <input type="url" name="team2_logo" value="{{ old('team2_logo', $stream->team2_logo) }}" placeholder="Flag/Logo URL" class="w-full bg-gray-950/40 border border-gray-800 focus:border-cyan-500 text-white rounded-lg px-2.5 py-1.5 outline-none text-xs">
                    </div>
                </div>
            </div>
        </div>
    ...
    <div class="glass-panel p-6 rounded-3xl shadow-xl">
        ...
        <div id="servers-container" class="space-y-4">
            ...
            @forelse($stream->servers as $index => $server)
                <div class="server-row p-4 bg-gray-950/35 border border-gray-800 rounded-2xl relative space-y-4">
                    <div class="grid grid-cols-1 sm:grid-cols-12 gap-4">
                        <div class="sm:col-span-3">
                            ...
                            <input type="text" name="servers[{{ $index }}][name]" value="{{ $server->name }}" required class="w-full bg-gray-950/60 border border-gray-800 text-white rounded-xl px-3 py-2 outline-none text-xs">
                        </div>
                        <div class="sm:col-span-2">
                            ...
                            <select name="servers[{{ $index }}][stream_type]" required class="w-full bg-gray-950/60 border border-gray-800 text-white rounded-xl px-3 py-2 outline-none text-xs">
                                <option value="iframe" {{ $server->stream_type == 'iframe' ? 'selected' : '' }}>Iframe / Web</option>
                                <option value="m3u8" {{ $server->stream_type == 'm3u8' ? 'selected' : '' }}>M3U8 / HLS</option>
                            </select>
                        </div>
                        <div class="sm:col-span-5">
                            <label class="text-xs text-gray-500 block mb-1">Streaming URL</label>
                            <input type="text" name="servers[{{ $index }}][url]" value="{{ $server->url }}" required class="w-full bg-gray-950/60 border border-gray-800 text-white rounded-xl px-3 py-2 outline-none text-xs">
                        </div>
                        <div class="sm:col-span-1">
                            <label class="text-xs text-gray-500 block mb-1">Order</label>
                            <input type="number" name="servers[{{ $index }}][order]" value="{{ $server->order }}" required class="w-full bg-gray-950/60 border border-gray-800 text-white rounded-xl px-3 py-2 outline-none text-xs">
                        </div>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 border-t border-gray-800/40 pt-3">
                        <div>
                            ...
                            <input type="text" name="servers[{{ $index }}][http_referer]" value="{{ $server->http_referer }}" class="w-full bg-gray-950/40 border border-gray-800/80 text-white rounded-xl px-3 py-1.5 outline-none text-xs" placeholder="e.g. https://executeandship.com/">
                        </div>
                        <div>
                            <label class="text-[10px] text-gray-500 block mb-1">Custom Origin Header (Optional)</label>
                            <input type="text" name="servers[{{ $index }}][http_origin]" value="{{ $server->http_origin }}" class="w-full bg-gray-950/40 border border-gray-800/80 text-white rounded-xl px-3 py-1.5 outline-none text-xs" placeholder="e.g. https://executeandship.com">
                        </div>
                    </div>
                </div>
            @empty
                <!-- Fallback to single blank row if none (should not occur) -->
                <div class="server-row grid grid-cols-1 sm:grid-cols-12 gap-4 p-4 bg-gray-950/35 border border-gray-800 rounded-2xl relative">
                    <div class="sm:col-span-3">
                        <label class="text-xs text-gray-500 block mb-1">Server Name</label>
...
@push('scripts')
<script>
    let serverIndex = {{ count($stream->servers) > 0 ? count($stream->servers) : 1 }};
