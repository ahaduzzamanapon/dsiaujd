...
<body class="min-h-screen flex flex-col md:flex-row">
    ...
    <aside class="w-full md:w-64 glass-panel md:min-h-screen flex flex-col z-30">
        ...
        <nav id="mobile-nav" class="hidden md:flex flex-col flex-1 p-4 space-y-2">
            ...
            <a href="{{ route('admin.promo-banners.index') }}" class="sidebar-link flex items-center space-x-3 px-4 py-3 rounded-lg text-gray-300 {{ Request::routeIs('admin.promo-banners.*') ? 'active' : '' }}">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"/></svg>
                <span>Promo Banners</span>
            </a>
            <a href="{{ route('admin.review-queue.index') }}" class="sidebar-link flex items-center justify-between px-4 py-3 rounded-lg text-gray-300 {{ Request::routeIs('admin.review-queue.*') ? 'active' : '' }}">
                <div class="flex items-center space-x-3">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-3 7h3m-3 4h3m-6-4h.01M9 16h.01"/></svg>
                    <span>Review Queue</span>
                </div>
                @php
                    $reviewCount = \App\Models\PendingStream::count();
                @endphp
                @if($reviewCount > 0)
                    <span class="px-2.5 py-0.5 text-xs font-bold rounded-full bg-cyan-500 text-gray-950">{{ $reviewCount }}</span>
                @endif
            </a>
